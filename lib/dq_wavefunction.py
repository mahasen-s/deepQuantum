...
class wavefunction():
    ...
    def __init__(self,sess,nn_type='deep',input_num=1, hidden_num=None, layers_num=None, activation_function='sigmoid'):
        ...
        if conf.DTYPE == tf.complex64 or conf.DTYPE ==  tf.complex128:
            t_dtype = tf.float32 if conf.DTYPE == tf.complex64 else tf.float64
            tf_weight_gen = lambda x: tf.complex( tf.Variable(tf.random_normal(x, dtype=t_dtype)), tf.Variable(tf.random_normal(x, dtype=t_dtype)) )
            tf_bias_gen = lambda x: tf.complex( tf.Variable(tf.random_normal(x, dtype=t_dtype)), tf.Variable(tf.random_normal(x, dtype=t_dtype)) )
        else:
    ...
    def buildOp(self,input_states):
        ...
        if self.nn_type == 'shallow':
            ...
            psi = tf.add(tf.einsum('ij,ajk->ak',self.weights[-1],curr_net),self.biases[-1]);
